...
function buildConnectionConfig(client, connectionOrConfig) {
    const useSsl = isTruthy(process.env.DATABASE_SSL) || isTruthy(process.env.DB_SSL);
    const rejectUnauthorized = !/^(0|false|no|off)$/i.test((process.env.DATABASE_SSL_REJECT_UNAUTHORIZED || process.env.DB_SSL_REJECT_UNAUTHORIZED || "true").trim());
